...
const { getTokenBalance } = require("./solanaNative");
...
const transferToken = async ({
  ...
  try {
    ...
    // get source balance
    const sourceBalance = await getTokenBalance(source.address);
    console.log("Source Balance:", sourceBalance);

    // Check if source has enough balance
    const totalAmount =
      amount + (destinationTwoAmount || 0) + (destinationThreeAmount || 0);

    if (sourceBalance < totalAmount) {
      return {
        success: false,
        typeLabel: "InsufficientBalance",
        data: {
          sourceBalance
        }
      };
